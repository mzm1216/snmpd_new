...
int
notifyTable_unregister_notifications2(int major, int minor,
                                   void *serverarg, void *clientarg)
{
    ...
    if(ptr->nameData)
    {
        memcpy(buf, ptr->nameData, ptr->nameLen);
		bufLen  = ptr->nameLen;
    }
	else
	{
	    printf("name = NULL %s %d\r\n",__FUNCTION__,__LINE__);
	    return -1;
	}
	snmpTargetAddrTable_rem(ptr);
     ...
     * param 
     */
    pptr = get_paramEntry(buf);
	if(pptr != NULL)
	{
	    snmpTargetParamTable_rem(pptr);
	}
	else
	{
	    printf("pptr = NULL %s %d\r\n",__FUNCTION__,__LINE__);
		return -1;
	}
    /*
...
init_snmpNotifyTable(void)
{
    ...
    snmp_register_callback(SNMP_CALLBACK_APPLICATION,
                           ...
                           notifyTable_unregister_notifications2, NULL);
...
shutdown_snmpNotifyTable(void)
{
    ...
    snmp_unregister_callback(SNMP_CALLBACK_APPLICATION,
                             SNMPD_CALLBACK_UNREGISTER_NOTIFICATIONS,
                             notifyTable_unregister_notifications2, NULL, FALSE);
...
int
snmpNotifyTable_rem(char *notify_name)
{
    struct snmpNotifyTable_data *storagetmp = NULL, *storageDel = NULL;
	struct header_complex_index * NotifyTableStorage = NULL, *hciptr = NULL;
	struct header_complex_index **NotifyTableStorageAddr = NULL;
    netsnmp_variable_list *vars = NULL;
	int newlen;
	oid name[33];
	int i;

    DEBUGMSGTL(("snmpNotifyTable", "reming data...  "));
	...
	newlen = (int)strlen(notify_name);

	NotifyTableStorage = snmpNotifyTableStorage;
	NotifyTableStorageAddr = &snmpNotifyTableStorage;

	if(newlen > 32)
	{
	    printf("name too long %s %d\r\n",__FUNCTION__,__LINE__);
		return -1;
	}

	for(i = 0; i < newlen; i++)
	{
	    name[i] = (oid)notify_name[i];
	}

	if((storagetmp = (struct snmpNotifyTable_data *)header_complex
		((struct header_complex_index *)NotifyTableStorage, NULL, name,
	...
	hciptr = header_complex_find_entry(NotifyTableStorage, storagetmp);
	storageDel = (struct snmpNotifyTable_data *)header_complex_extract_entry
		     ((struct header_complex_index **)NotifyTableStorageAddr, hciptr);
	if(storagetmp->snmpNotifyName)
	{
	    printf("snmpNotifyName %s %s %d\r\n",storagetmp->snmpNotifyName,__FUNCTION__,__LINE__);
