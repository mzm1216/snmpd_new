...
#  include "mib_module_inits.h"
    ...
    if (once == 0) {
		...
		snmp_log(LOG_DEBUG,"##################################################\r\n");
		snmp_log(LOG_DEBUG,"Snmpd_Version[%d.%d.%d]\r\n",SNMPD_VERSION_X,SNMPD_VERSION_Y,SNMPD_VERSION_Z);
		snmp_log(LOG_DEBUG,"%s %s \r\n", __DATE__, __TIME__);
		snmp_log(LOG_DEBUG,"##################################################\r\n");
		
		printf("##################################################\r\n");
		printf("Snmpd_Version[%d.%d.%d]\r\n",SNMPD_VERSION_X,SNMPD_VERSION_Y,SNMPD_VERSION_Z);
		printf("%s %s \r\n", __DATE__, __TIME__);
		printf("##################################################\r\n");
	 init_vigorPubMIBObject();
	 
#if (VT_3308_MIB == TRUE)

		printf("#############VT_3308_MIB#######\r\n");
		snmp_log(LOG_DEBUG,"#############VT_3308_MIB#######\r\n");

        init_vt3308MIBObject();
#endif

#if (VT_3830_MIB == TRUE)
		printf("#############VT_3830_MIB#######\r\n");
		snmp_log(LOG_DEBUG,"#############VT_3830_MIB#######\r\n");

        init_vt3830MIBObject();
#endif
#if (VT_3888_MIB == TRUE)
		printf("#############VT_3888_MIB#######\r\n");
		snmp_log(LOG_DEBUG,"#############VT_3888_MIB#######\r\n");
        init_vt3888MIBObject();

#endif
        /*由于消息队列后来改为由业务进程创建，所以该函数内容为空，可以注销掉*/
	 init_vigorMsgQueue();
