...
#endif
{
    ...
    signal(SIGALRM, SNMP_TRAP_SendHeartBeatTrap);
...
/*set the timer to send heart beat trap*/
    ...
    if(setitimer(ITIMER_REAL, &tick, NULL) < 0)
            snmp_log(LOG_DEBUG,"Set timer failed!\n");
    ...
    reselect:
        ...
        snmp_log(LOG_DEBUG,"snmpd/select, select( numfds=%d, ..., tvp=%p)\n",
                    numfds, tvp);
