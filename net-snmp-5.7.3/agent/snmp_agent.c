...
void init_vigorMsgQueue(void)
{
#if 0
    int msqid = -1;
    msqid = msgget((key_t)SNMP_MSGKEY, IPC_EXCL);  /*检查消息队列是否存在*/  
    if(msqid < 0)
    {
        msqid = msgget((key_t)SNMP_MSGKEY, IPC_CREAT|0666);/*创建消息队列*/  
        if(msqid <0)
        {  
            printf("%s:%d  failed to create msq\r\n", __FUNCTION__, __LINE__);  
            return;  
        }
		else
		{
		    printf("%s:%d  snmp queue id = %d\r\n", __FUNCTION__, __LINE__, msqid); 
		}
    } 
#endif
#if 0
	...
	{
	    snmp_log(LOG_DEBUG,"%s:%d  failed to create mutex\r\n", __FUNCTION__, __LINE__);  
        return;
	}


	if(pthread_mutex_init(&mut_trap,NULL) != 0)
	{
	    snmp_log(LOG_DEBUG,"%s:%d  failed to create mut_trap\r\n", __FUNCTION__, __LINE__);  
        return;
	}
#endif
}

int send_snmpMsg(QUEUE_MSG_T *p_msg)
{
    int msqid = -1; 
    int ret_value = MSG_GENERROR;

	msqid = msgget((key_t)SNMP_MSGKEY_SEND, IPC_EXCL);/*检查消息队列是否存在 */ 
	if(msqid < 0)
	{
	    snmp_log(LOG_DEBUG,"%s:%d  fail to get msg queue msqid[%d]\r\n", __FUNCTION__, __LINE__,msqid);
		return MSG_GENERROR;
	}
	else
	{
	    snmp_log(LOG_DEBUG,"%s:%d  app queue id = %d\r\n", __FUNCTION__, __LINE__, msqid);
	    snmp_log(LOG_DEBUG,"%s:%d  msg_type[%x]  oper_code[0x%x]\r\n", __FUNCTION__, __LINE__, p_msg->msg_type,p_msg->snmp_msg.oper_code); 
	}
	
	ret_value = msgsnd(msqid, p_msg, sizeof(SNMP_MSG_T), IPC_NOWAIT);
	if(ret_value == MSG_GENERROR)
	{
	    snmp_log(LOG_DEBUG,"%s:%d  fail to send msg to queue %d\r\n", __FUNCTION__, __LINE__, errno);
		return MSG_GENERROR;
	}
...
int clean_AckRecvMsgQueue(QUEUE_MSG_T *p_msg)
{
    int msqid = -1; 
    int ret_value = MSG_OK;
	static int count = 0;
	
	//	msqid = msgget((key_t)SNMP_MSGKEY_RECV, IPC_EXCL);/*检查消息队列是否存在 */ 
	msqid = msgget((key_t)APP_ACK_MSGKEY, IPC_EXCL);/*检查ack消息队列是否存在 */ 
	if(msqid < 0)
	{
	    snmp_log(LOG_DEBUG,"%s:%d  fail to get msg queue msqid[%d]\r\n", __FUNCTION__, __LINE__,msqid);
		return MSG_GENERROR;
	...
	while(ret_value != MSG_GENERROR)
	{
	
//		msqid = msgget((key_t)APP_ACK_MSGKEY, IPC_EXCL);/*检查ack消息队列是否存在 */ 
//			if(msqid < 0)
//			{
//				snmp_log(LOG_DEBUG,"%s:%d  fail to get msg queue msqid1[%d]\r\n", __FUNCTION__, __LINE__,msqid);
//				return MSG_GENERROR;
//			}
	    ret_value = msgrcv(msqid, p_msg, sizeof(SNMP_MSG_T), CMD_ACK, IPC_NOWAIT);

	}
	return MSG_OK;
}

int recv_ack_snmpMsg(QUEUE_MSG_T *p_msg)
{
    int msqid = -1; 
    int ret_value = MSG_GENERROR;
	int count = 0;

	msqid = msgget((key_t)APP_ACK_MSGKEY, IPC_EXCL);/*检查消息队列是否存在 */ 
	if(msqid < 0)
	{
	    snmp_log(LOG_DEBUG,"%s:%d  fail to get msg queue msqid[%d]\r\n", __FUNCTION__, __LINE__,msqid);
		return MSG_GENERROR;
	...
	{
	    snmp_log(LOG_DEBUG,"%s:%d  snmp queue id = %d\r\n", __FUNCTION__, __LINE__, msqid); 
	}

	while((ret_value == MSG_GENERROR) && (count < 10))
	{
			
	    ret_value = msgrcv(msqid, p_msg, sizeof(SNMP_MSG_T), CMD_ACK, IPC_NOWAIT);
		snmp_log(LOG_DEBUG,"%s:%d ret_value id = %d\r\n", __FUNCTION__, __LINE__, ret_value); 
		if(ret_value == MSG_GENERROR)
		{
		    usleep(200000);
			count ++;
    ...
    snmp_log(LOG_DEBUG,"%s:%d  fail to recv msg from queue\r\n", __FUNCTION__, __LINE__);
	return MSG_TIMEOUT;
}
...
{
    int ret_value = MSG_GENERROR;
	unsigned long   oper_code = 0;        /*操作码*/
	QUEUE_MSG_T msg = {0};

    //pthread_mutex_lock(&mut);
	
    /*clean the recv msg queue first*/
	clean_AckRecvMsgQueue(&msg);

	oper_code = p_msg->snmp_msg.oper_code;

	ret_value = send_snmpMsg(p_msg);
	if(ret_value != MSG_OK)
	{
	    snmp_log(LOG_DEBUG,"%s:%d  fail to send msg\r\n", __FUNCTION__, __LINE__);
		//pthread_mutex_unlock(&mut);
		return ret_value;
	}

	snmp_log(LOG_DEBUG,"%s:%d  success to send msg\r\n", __FUNCTION__, __LINE__);
	
	ret_value = recv_ack_snmpMsg(p_msg);
	if(ret_value != MSG_OK)
	{
	    snmp_log(LOG_DEBUG,"%s:%d  fail to recv msg\r\n", __FUNCTION__, __LINE__);
		//pthread_mutex_unlock(&mut);
		return ret_value;
	}
	else
	{
	    snmp_log(LOG_DEBUG,"%s:%d  recv msg: msgtype = %x, oper_code = %x, msg_data = %s\r\n", __FUNCTION__, __LINE__,
			                          p_msg->msg_type,
			                          p_msg->snmp_msg.oper_code,
			                          p_msg->snmp_msg.u_data.msg_data_str);
	}

	if(p_msg->snmp_msg.oper_code != oper_code)
	{
	    snmp_log(LOG_DEBUG,"%s:%d  error oper code from ack msg\r\n", __FUNCTION__, __LINE__);
		//pthread_mutex_unlock(&mut);
		return MSG_WRONGOPERCODE;
	}

	if(p_msg->snmp_msg.error_code != MSG_OK)
	{
	    snmp_log(LOG_DEBUG,"%s:%d  app oper error\r\n", __FUNCTION__, __LINE__);
		//pthread_mutex_unlock(&mut);
		return MSG_APPOPERERROR;
	}
...
#if 0
static int snmp_sendMsg(QUEUE_TRAP_MSG_T *p_msg)
{
    int msqid = -1; 
    int ret_value = MSG_GENERROR;
	...
	if(msqid < 0)
	{
	    printf("%s:%d  fail to get msg queue\r\n", __FUNCTION__, __LINE__);
		return MSG_GENERROR;
	}
	
	ret_value = msgsnd(msqid, p_msg, sizeof(SNMP_TRAP_MSG_T), IPC_NOWAIT);
	if(ret_value == MSG_GENERROR)
	{
	    printf("%s:%d  fail to send msg to queue\r\n", __FUNCTION__, __LINE__);
		return MSG_GENERROR;
	}
...
static int snmp_cleanRecvMsgQueue(QUEUE_TRAP_MSG_T *p_msg)
{
    int msqid = -1; 
    int ret_value = MSG_OK;
	static int count = 0;
	...
	while(ret_value != MSG_GENERROR)
	{
	    ret_value = msgrcv(msqid, p_msg, sizeof(SNMP_TRAP_MSG_T), CMD_ACK, IPC_NOWAIT);
	}
	return MSG_OK;
}

static int snmp_trapsendMsg(QUEUE_TRAP_MSG_T *p_msg)
{
    int msqid = -1; 
    int ret_value = MSG_GENERROR;
...
static int snmp_recvMsg(QUEUE_TRAP_MSG_T *p_msg)
{
    int msqid = -1; 
    int ret_value = MSG_GENERROR;
	static int count = 0;
    ...
    printf("%s:%d  fail to recv msg from queue\r\n", __FUNCTION__, __LINE__);
	return MSG_TIMEOUT;
}

int snmp_processMsg(QUEUE_TRAP_MSG_T *p_msg)
{
    int ret_value = MSG_GENERROR;
	unsigned long   oper_code = 0;        /*操作码*/
	QUEUE_TRAP_MSG_T msg = {0};

    /*clean the recv msg queue first*/
    snmp_cleanRecvMsgQueue(&msg);
	
	oper_code = p_msg->snmp_msg.oper_code;
	ret_value = snmp_sendMsg(p_msg);
	if(ret_value != MSG_OK)
	{
	    printf("%s:%d  fail to send msg\r\n", __FUNCTION__, __LINE__);
		return ret_value;
	}

	ret_value = snmp_recvMsg(p_msg);
	if(ret_value != MSG_OK)
	{
	    printf("%s:%d  fail to recv msg\r\n", __FUNCTION__, __LINE__);
		return ret_value;
	}

	if(p_msg->snmp_msg.oper_code != oper_code)
	{
	    printf("%s:%d  error oper code from ack msg\r\n", __FUNCTION__, __LINE__);
		return MSG_WRONGOPERCODE;
	}

	if(p_msg->snmp_msg.error_code != MSG_OK)
	{
	    printf("%s:%d  app oper error\r\n", __FUNCTION__, __LINE__);
		return MSG_APPOPERERROR;
	}
