...
netsnmp_session *snmp_sess_find_by_addr(struct sockaddr_in *addr)
{
    struct session_list *slp = NULL;
    unsigned char  remote[6] = {0};
    if(addr == NULL)
    {
        return NULL;
    }
	
	memcpy(remote, (u_char *) & (addr->sin_addr.s_addr), 4);
    remote[4] = (ntohs(addr->sin_port) & 0xff00) >> 8;
    remote[5] = (ntohs(addr->sin_port) & 0x00ff) >> 0;
    ...
    {                           /*MTCRITICAL_RESOURCE */
        ...
        for (slp = Sessions; slp; slp = slp->next) 
		{
			...
			{
                if ((slp->transport->remote_length == 6) && (slp->transport->remote != NULL))
                {
                    if(0 == memcmp(remote, slp->transport->remote, 6))
                    {
                        snmp_res_unlock(MT_LIBRARY_ID, MT_LIB_SESSION);
						return slp->session;
                    }
...
static oid snmp_trap_vt3830mib_oid[] = {1,3,6,1,4,1,259,1,1,1};
static uint32_t len_snmp_trap_vt3830mib_oid = OID_LENGTH(snmp_trap_vt3830mib_oid);

static oid snmp_trap_vt3380mib_oid[] = {1,3,6,1,4,1,259,1,2,1};
static uint32_t len_snmp_trap_vt3380mib_oid = OID_LENGTH(snmp_trap_vt3380mib_oid);


/****************************clk distribut***********************************/
static oid snmp_trap_vt3888mib_oid[] = {1,3,6,1,4,1,259,1,4,1};
static uint32_t len_snmp_trap_vt3888mib_oid = OID_LENGTH(snmp_trap_vt3888mib_oid);
static oid vt3888Notifications_oid[] = {1,3,6,1,4,1,259,1,4,2};
static uint32_t len_vt3888Notifications_oid = OID_LENGTH(vt3888Notifications_oid);

static oid cdd_Clk_Source_Trap_oid[] = {1,3,6,1,4,1,259,1,4,2,1};
static uint32_t len_cdd_Clk_Source_Trap_oid = OID_LENGTH(cdd_Clk_Source_Trap_oid);

static oid cdd_GPS_Lock_Trap_oid[] = {1,3,6,1,4,1,259,1,4,2,2};
static uint32_t len_cdd_GPS_Lock_Trap_oid = OID_LENGTH(cdd_GPS_Lock_Trap_oid);

static oid cdd_PTP_Lock_Trap_oid[] = {1,3,6,1,4,1,259,1,4,2,3};
static uint32_t len_cdd_PTP_Lock_Trap_oid = OID_LENGTH(cdd_PTP_Lock_Trap_oid);

static oid cdd_FPGA_Work_State_Trap_oid[] = {1,3,6,1,4,1,259,1,4,2,4};
static uint32_t len_cdd_FPGA_Work_State_Trap_oid = OID_LENGTH(cdd_FPGA_Work_State_Trap_oid);


static oid cdd_Net_Link_Trap_oid[] = {1,3,6,1,4,1,259,1,4,2,5};
static uint32_t len_cdd_Net_Link_Trap_oid = OID_LENGTH(cdd_Net_Link_Trap_oid);

static oid cdd_Omap_Run_Trap_oid[] = {1,3,6,1,4,1,259,1,4,2,6};
static uint32_t len_cdd_Omap_Run_Trap_oid = OID_LENGTH(cdd_Omap_Run_Trap_oid);
/***************************************************************/



static oid vt3830Notifications_oid[] = {1,3,6,1,4,1,259,1,1,2};
static uint32_t len_vt3830Notifications_oid = OID_LENGTH(vt3830Notifications_oid);


static oid bts_Alarm_Status_Trap_oid[] = {1,3,6,1,4,1,259,1,1,2,1};
static uint32_t len_bts_Alarm_Status_Trap_oid = OID_LENGTH(bts_Alarm_Status_Trap_oid);

static oid bts_LinkState_Trap_oid[] = {1,3,6,1,4,1,259,1,1,2,2};
static uint32_t len_bts_LinkState_Trap_oid = OID_LENGTH(bts_LinkState_Trap_oid);

static oid bts_ExtClock_Trap_oid[] = {1,3,6,1,4,1,259,1,1,2,3};
static uint32_t len_bts_ExtClock_Trap_oid = OID_LENGTH(bts_ExtClock_Trap_oid);

static oid bts_TxState_Trap_oid[] = {1,3,6,1,4,1,259,1,1,2,4};
static uint32_t len_bts_TxState_Trap_oid = OID_LENGTH(bts_TxState_Trap_oid);

static oid vt3308Notifications_oid[] = {1,3,6,1,4,1,259,1,2,2};
static uint32_t len_vt3308Notifications_oid = OID_LENGTH(vt3308Notifications_oid);

static oid vigorPubNotifications_oid[] = {1,3,6,1,4,1,259,1,3,2};
static uint32_t len_vigorPubNotifications_oid = OID_LENGTH(vigorPubNotifications_oid);

static oid vigor_Heartbeat_Trap_oid[] = {1,3,6,1,4,1,259,1,3,2,1};
static uint32_t len_vigor_Heartbeat_Trap_oid = OID_LENGTH(vigor_Heartbeat_Trap_oid);

static oid ctc_SlotState_Trap_oid[] = {1,3,6,1,4,1,259,1,2,2,1};
static uint32_t len_ctc_SlotState_Trap_oid = OID_LENGTH(ctc_SlotState_Trap_oid);

static oid ctc_RepeaterState_Trap_oid[] = {1,3,6,1,4,1,259,1,2,2,2};
static uint32_t len_ctc_RepeaterState_Trap_oid = OID_LENGTH(ctc_RepeaterState_Trap_oid);

static oid ctc_LinkState_Trap_oid[] = {1,3,6,1,4,1,259,1,2,2,3};
static uint32_t len_ctc_LinkState_Trap_oid = OID_LENGTH(ctc_LinkState_Trap_oid);


static oid ID_btsIpAddress[] = { 1, 3, 6, 1, 4, 1, 259, 1, 1, 1, 1, 0 };
static uint32_t len_btsIpAddress = OID_LENGTH(ID_btsIpAddress);

static oid ID_btsModel[] = { 1, 3, 6, 1, 4, 1, 259, 1, 1, 1, 11, 0 };
static uint32_t len_btsModel = OID_LENGTH(ID_btsModel);

//static oid ID_btsAlarmStatus[] = {1,3,6,1,4,1,259,1,1,1,30,0};
//static uint32_t len_btsAlarmStatus = OID_LENGTH(ID_btsAlarmStatus);
static oid ID_vigorSource[] = {1,3,6,1,4,1,259,1,3,1,6,0};
static uint32_t len_vigorSource = OID_LENGTH(ID_vigorSource);

static oid ID_btsAlarmState[] = {1,3,6,1,4,1,259,1,1,1,24,0};
static uint32_t len_btsAlarmState = OID_LENGTH(ID_btsAlarmState);

static oid ID_btsLinkState[] = {1,3,6,1,4,1,259,1,1,1,25,0};
static uint32_t len_btsLinkState = OID_LENGTH(ID_btsLinkState);

static oid ID_btsExtClock[] = {1,3,6,1,4,1,259,1,1,1,26,0};
static uint32_t len_btsExtClock = OID_LENGTH(ID_btsExtClock);

static oid ID_btsTxState[] = {1,3,6,1,4,1,259,1,1,1,27,0};
static uint32_t len_btsTxState = OID_LENGTH(ID_btsTxState);

static oid ID_btsRealTxPower[] = { 1, 3, 6, 1, 4, 1, 259, 1, 1, 1, 15, 0};
static uint32_t len_btsRealTxPower = OID_LENGTH(ID_btsRealTxPower);

static oid ID_btsVswr[] = { 1, 3, 6, 1, 4, 1, 259, 1, 1, 1, 16, 0};
static uint32_t len_btsVswr = OID_LENGTH(ID_btsVswr);

static oid ID_btsRssi[] = { 1, 3, 6, 1, 4, 1, 259, 1, 1, 1, 17, 0};
static uint32_t len_btsRssi = OID_LENGTH(ID_btsRssi);

static oid ID_btsEnvirTemperature[] = { 1, 3, 6, 1, 4, 1, 259, 1, 1, 1, 18, 0};
static uint32_t len_btsEnvirTemperature = OID_LENGTH(ID_btsEnvirTemperature);

static oid ID_btsPaTemperature[] = { 1, 3, 6, 1, 4, 1, 259, 1, 1, 1, 19, 0};
static uint32_t len_btsPaTemperature = OID_LENGTH(ID_btsPaTemperature);

static oid ID_btsDcVoltage[] = { 1, 3, 6, 1, 4, 1, 259, 1, 1, 1, 20, 0};
static uint32_t len_btsDcVoltage = OID_LENGTH(ID_btsDcVoltage);

static oid ID_btsBatVoltage[] = { 1, 3, 6, 1, 4, 1, 259, 1, 1, 1, 21, 0};
static uint32_t len_btsBatVoltage = OID_LENGTH(ID_btsBatVoltage);

static oid ID_btsFanSpeed[] = { 1, 3, 6, 1, 4, 1, 259, 1, 1, 1, 22, 0};
static uint32_t len_btsFanSpeed = OID_LENGTH(ID_btsFanSpeed);

static oid ID_sccSlotNum[] = {1,3,6,1,4,1,259,1,2,1,10,0};
static uint32_t len_sccSlotNum = OID_LENGTH(ID_sccSlotNum);

static oid ID_sccBusy[] = {1,3,6,1,4,1,259,1,2,1,11,0};
static uint32_t len_sccBusy = OID_LENGTH(ID_sccBusy);

static oid ID_sccCaller[] = {1,3,6,1,4,1,259,1,2,1,12,0};
static uint32_t len_sccCaller = OID_LENGTH(ID_sccCaller);

static oid ID_sccCalled[] = {1,3,6,1,4,1,259,1,2,1,13,0};
static uint32_t len_sccCalled = OID_LENGTH(ID_sccCalled);

static oid ID_sccRtpIpAddress[] = {1,3,6,1,4,1,259,1,2,1,14,0};
static uint32_t len_sccRtpIpAddress = OID_LENGTH(ID_sccRtpIpAddress);

static oid ID_sccCallInfo[] = {1,3,6,1,4,1,259,1,2,1,15,0};
static uint32_t len_sccCallInfo = OID_LENGTH(ID_sccCallInfo);

static oid ID_sccLdsIpAddress[] = { 1,3,6,1,4,1,259,1,2,1,1,0 };
static uint32_t len_sccLdsIpAddress = OID_LENGTH(ID_sccLdsIpAddress);

static oid ID_mscBtsIpAddress[] = {1,3,6,1,4,1,259,1,2,1,16,0};
static uint32_t len_mscBtsIpAddress = OID_LENGTH(ID_mscBtsIpAddress);

static oid ID_mscBtsState[] = {1,3,6,1,4,1,259,1,2,1,17,0};
static uint32_t len_mscBtsState = OID_LENGTH(ID_mscBtsState);

static oid ID_mscLinkState[] = {1,3,6,1,4,1,259,1,2,1,18,0};
static uint32_t len_mscLinkState = OID_LENGTH(ID_mscLinkState);

static oid ID_sccTalker[] = {1,3,6,1,4,1,259,1,2,1,19,0};
static uint32_t len_sccTalker = OID_LENGTH(ID_sccTalker);

static oid ID_sccBSID[] = {1,3,6,1,4,1,259,1,2,1,20,0};
static uint32_t len_sccBSID = OID_LENGTH(ID_sccBSID);

static oid ID_vigorEquipmentType[] = {1,3,6,1,4,1,259,1,3,1,5,0};
static uint32_t len_vigorEquipmentType = OID_LENGTH(ID_vigorEquipmentType);

static oid ID_vigorDeviceState[] = {1,3,6,1,4,1,259,1,3,1,7,0};
static uint32_t len_vigorDeviceState = OID_LENGTH(ID_vigorDeviceState);




Trap_vigorHeartbeat_T   trap_heartbeat = {0};
int time_index = 0;
BOOL SNMP_TRAP_SendTrap(TRAP_EVENT_TrapData_T *trap_data_p);
void *SNMP_TRAP_ProcessTrapMsg(void *arg);
...
void *SNMP_TRAP_ProcessTrapMsg(void *arg)
{
    ...
    QUEUE_MSG_T msg = {0};
    TRAP_EVENT_TrapData_T trap_data_p = {0};

    snmp_log(LOG_DEBUG,"%s:%d  #######\r\n", __FUNCTION__, __LINE__);
        
    msqid = msgget((key_t)SNMP_MSGKEY_RECV, IPC_EXCL);/*\BC\EC\B2\E9\CF\FBϢ\B6\D3\C1\D0\CAǷ\F1\B4\E6\D4\DA */ 
    if(msqid < 0)
    {
        snmp_log(LOG_DEBUG,"%s:%d  msgget fail\r\n", __FUNCTION__, __LINE__);
        //usleep(5000000);
        //msqid = msgget((key_t)SNMP_MSGKEY_RECV, IPC_EXCL);
        return;
    }
    
 //   while ((snmp_sess_is_null() == FALSE) && (count < 20))
		
    while ((count < 20))
    //if(snmp_sess_is_null() == FALSE)
    {
//	     msqid = msgget((key_t)SNMP_MSGKEY_RECV, IPC_EXCL);/*\BC\EC\B2\E9\CF\FBϢ\B6\D3\C1\D0\CAǷ\F1\B4\E6\D4\DA */ 
//	    if(msqid < 0)
//	    {
...
//        ret_value = msgrcv(msqid, &msg, sizeof(QUEUE_MSG_T), CMD_TRAP, IPC_NOWAIT);
		
        ret_value = msgrcv(msqid, &msg, sizeof(QUEUE_MSG_T), CMD_TRAP, IPC_NOWAIT);
        if(ret_value == MSG_GENERROR)
        {
            //pthread_mutex_unlock(&mut);
			snmp_log(LOG_DEBUG,"%s:%d  recv error\r\n", __FUNCTION__, __LINE__);
        ...
        {
            snmp_log(LOG_DEBUG,"%s:%d  recv trap msg: msgtype = %x, oper_code = %x, msg_data = %s,ret_value=%d \r\n", __FUNCTION__, __LINE__,
                                      msg.msg_type,
                                      msg.snmp_msg.oper_code,
                                      msg.snmp_msg.u_data.msg_data_str,
                                      ret_value);
        }
        //pthread_mutex_unlock(&mut);
        count++;
        memset(&trap_data_p, 0, sizeof(trap_data_p));
        trap_data_p.flag = TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP;
        trap_data_p.trap_type = msg.snmp_msg.oper_code;
        memcpy(&(trap_data_p.u), &(msg.snmp_msg.u_data), sizeof(trap_data_p.u));
        SNMP_TRAP_SendTrap(&trap_data_p);
...
void SNMP_TRAP_SendHeartBeatTrap(int signo)
{
	...
	trap_data_p.flag = TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP;
	trap_data_p.trap_type = SNMP_TRAP_QUEUE_BTSALARMSTATE;
	//trap_data_p.u.bts_equipment_trap.btsAlarmStatus[0]= 1;
	trap_data_p.u.bts_equipment_trap.btsAlarmType[0]= 1;
	trap_data_p.u.bts_equipment_trap.vigorSource[0]= 1;
	SNMP_TRAP_SendTrap(&trap_data_p);

    memset(&trap_data_p, 0, sizeof(trap_data_p));
	trap_data_p.flag = TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP;
	trap_data_p.trap_type = SNMP_TRAP_QUEUE_BTSLINKSTATE;
	trap_data_p.u.bts_linkstate_trap.btsLinkState[0] = 1;
	trap_data_p.u.bts_linkstate_trap.vigorSource[0] = 1;
	SNMP_TRAP_SendTrap(&trap_data_p);

    memset(&trap_data_p, 0, sizeof(trap_data_p));
	trap_data_p.flag = TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP;
	trap_data_p.trap_type = SNMP_TRAP_QUEUE_BTSEXTCLOCK;
	trap_data_p.u.bts_extclock_trap.btsExtClock[0] = 1;
	trap_data_p.u.bts_extclock_trap.vigorSource[0] = 1;
	SNMP_TRAP_SendTrap(&trap_data_p);

	memset(&trap_data_p, 0, sizeof(trap_data_p));
	trap_data_p.flag = TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP;
	trap_data_p.trap_type = SNMP_TRAP_QUEUE_BTSTXSTATE;
	trap_data_p.u.bts_txstate_trap.btsTxState[0] = 1;
	trap_data_p.u.bts_txstate_trap.vigorSource[0] = 1;
	SNMP_TRAP_SendTrap(&trap_data_p);

    memset(&trap_data_p, 0, sizeof(trap_data_p));
	trap_data_p.flag = TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP;
	trap_data_p.trap_type = SNMP_TRAP_QUEUE_SCCSLOTSTATE;
	trap_data_p.u.msc_slotstate_trap.sccSlotNum[0] = 1;
	trap_data_p.u.msc_slotstate_trap.sccCaller[0] = 1;
	trap_data_p.u.msc_slotstate_trap.sccCalled[0] = 1;
	memcpy(trap_data_p.u.msc_slotstate_trap.sccRtpIpAddress, "192.168.113.32",sizeof("192.168.113.32"));
	SNMP_TRAP_SendTrap(&trap_data_p);

	memset(&trap_data_p, 0, sizeof(trap_data_p));
	trap_data_p.flag = TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP;
	trap_data_p.trap_type = SNMP_TRAP_QUEUE_SCCBTSSTATE;
	trap_data_p.u.msc_btsstate_trap.sccBtsState[0]= 1;
	memcpy(trap_data_p.u.msc_btsstate_trap.sccBtsIpAddress, "192.168.113.32",sizeof("192.168.113.32"));
	SNMP_TRAP_SendTrap(&trap_data_p);

	memset(&trap_data_p, 0, sizeof(trap_data_p));
	trap_data_p.flag = TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP;
	trap_data_p.trap_type = SNMP_TRAP_QUEUE_SCCLINKSTATE;
	trap_data_p.u.msc_linkstate_trap.sccLinkState[0]= 1;
	SNMP_TRAP_SendTrap(&trap_data_p);

	memset(&trap_data_p, 0, sizeof(trap_data_p));
	trap_data_p.flag = TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP;
	trap_data_p.trap_type = SNMP_TRAP_QUEUE_SCCPTTSTATETRAP;
	trap_data_p.u.scc_PttState_Trap.sccTalker[0]= 1;
	SNMP_TRAP_SendTrap(&trap_data_p);
	...
	memset(&trap_data_p, 0, sizeof(trap_data_p));
	trap_data_p.flag = TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP;
	trap_data_p.trap_type = SNMP_TRAP_QUEUE_VIGORHEARTBEAT;
	trap_data_p.u.vigor_heartbeat_trap.equipType[0] = 1;
	memcpy(trap_data_p.u.vigor_heartbeat_trap.vigorDeviceState, "192.168.1.1", sizeof("192.168.1.1"));
	SNMP_TRAP_SendTrap(&trap_data_p);

	memset(&trap_data_p, 0, sizeof(trap_data_p));
	trap_data_p.flag = TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP;
	trap_data_p.trap_type = SNMP_TRAP_QUEUE_VIGORPROCABORT;
	trap_data_p.u.vigor_procabort_trap.equipType[0] = 3;
	memcpy(trap_data_p.u.vigor_procabort_trap.vigorDeviceState, "192.168.1.2", sizeof("192.168.1.2"));
	SNMP_TRAP_SendTrap(&trap_data_p);
#endif
    ...
    if(strlen(trap_heartbeat.vigorSource) == 0)
    {
		...
		msg.snmp_msg.oper_code = VIGOR_EQU_TYPE;
		ret_value = process_snmpMsg(&msg);
		if(ret_value != MSG_OK)
		{
		    snmp_log(LOG_DEBUG,"get VIGOR_EQU_TYPE fail %s %d\r\n",__FUNCTION__,__LINE__);
			//pthread_mutex_unlock(&mut_trap);
		    return;
		}
		else
		{
		    snmp_log(LOG_DEBUG,"get VIGOR_EQU_TYPE success %s %d\r\n",__FUNCTION__,__LINE__);
		    memcpy(trap_heartbeat.equipType, msg.snmp_msg.u_data.msg_data_str, strlen(msg.snmp_msg.u_data.msg_data_str));
		}

	    memset(&msg, 0, sizeof(msg));
		msg.msg_type = CMD_GET;
		msg.snmp_msg.oper_code = VIGOR_SOURCE;
		ret_value = process_snmpMsg(&msg);
		if(ret_value != MSG_OK)
		{
		    snmp_log(LOG_DEBUG,"get NET_IPADDR fail %s %d\r\n",__FUNCTION__,__LINE__);
			//pthread_mutex_unlock(&mut_trap);
		    return;
		}
		else
		{
		    snmp_log(LOG_DEBUG,"get NET_IPADDR success %s %d\r\n",__FUNCTION__,__LINE__);
		    snprintf(trap_heartbeat.vigorSource, PARAM_SIZE, "%s" ,msg.snmp_msg.u_data.msg_data_str);
		}

		snprintf(trap_heartbeat.vigorDeviceState, PARAM_SIZE, "reserved");
    }

	if(!snmp_varlist_add_variable( &vars, ID_vigorSource, len_vigorSource, ASN_OCTET_STR, 
		(const void *) (trap_heartbeat.vigorSource), strlen(trap_heartbeat.vigorSource)))
	{
	    snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
	    snmp_free_varbind(vars);
		//pthread_mutex_unlock(&mut_trap);
		return;
	}

    if(!snmp_varlist_add_variable( &vars, ID_vigorEquipmentType, len_vigorEquipmentType, ASN_OCTET_STR, 
		(const void *) (trap_heartbeat.equipType), strlen(trap_heartbeat.equipType)))
	{
	    snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
	    snmp_free_varbind(vars);
		//pthread_mutex_unlock(&mut_trap);
    ...
    if(!snmp_varlist_add_variable( &vars, ID_vigorDeviceState, len_vigorDeviceState, ASN_OCTET_STR, 
		(const void *) trap_heartbeat.vigorDeviceState, strlen(trap_heartbeat.vigorDeviceState)))
	{
	    snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
	    snmp_free_varbind(vars);
		//pthread_mutex_unlock(&mut_trap);
	...
	{
	    snmp_log(LOG_DEBUG,"send trap fail %s %d\r\n",__FUNCTION__,__LINE__);
	}
...
BOOL SNMP_TRAP_SendTrap(TRAP_EVENT_TrapData_T *trap_data_p)
{
    ...
    if(trap_data_p == NULL)
    {
        snmp_log(LOG_DEBUG,"trap_data_p = NULL %s %d \r\n",__FUNCTION__,__LINE__);
        return FALSE;
    }
...
//#if VT_3888_MIB
			...
			case SNMP_TRAP_QUEUE_CDD_CLK_SOURCE_TRAP:
			{
				...
				if(!snmp_varlist_add_variable( &vars, cdd_Clk_Source_Trap_oid, len_cdd_Clk_Source_Trap_oid, ASN_OCTET_STR, 
					(const void *) (pDynamicData->cddGPSClkState), strlen(pDynamicData->cddGPSClkState)))
				{
					snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
					snmp_free_varbind(vars);
					return FALSE;
				...
				if(!snmp_varlist_add_variable( &vars, ID_vigorSource, len_vigorSource, ASN_OCTET_STR, 
					(const void *) (pDynamicData->cddvigorSource), strlen(pDynamicData->cddvigorSource)))
				{
					snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
					snmp_free_varbind(vars);
					return FALSE;
			...
			{
				Trap_cddGPSLockState_T *pDynamicData = &(trap_data_p->u.cdd_GPSLockState_trap );
				memcpy(enterprise, vt3888Notifications_oid, len_vt3888Notifications_oid * sizeof(oid));
				enterprise_length = len_vt3888Notifications_oid;
				trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
				specific = 1;
				...
				if(!snmp_varlist_add_variable( &vars, cdd_GPS_Lock_Trap_oid, len_cdd_GPS_Lock_Trap_oid, ASN_OCTET_STR, 
					(const void *) (pDynamicData->cddGPSLockState), strlen(pDynamicData->cddGPSLockState)))
				{
					snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
					snmp_free_varbind(vars);
					return FALSE;
			...
			{
				Trap_cddPTPLockState_T *pDynamicData = &(trap_data_p->u.cdd_PTPLockState_trap );
				memcpy(enterprise, vt3888Notifications_oid, len_vt3888Notifications_oid * sizeof(oid));
				enterprise_length = len_vt3888Notifications_oid;
				trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
				specific = 1;
				...
				if(!snmp_varlist_add_variable( &vars, cdd_PTP_Lock_Trap_oid, len_cdd_PTP_Lock_Trap_oid, ASN_OCTET_STR, 
					(const void *) (pDynamicData->cddPTPLockState), strlen(pDynamicData->cddPTPLockState)))
				{
					snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
					snmp_free_varbind(vars);
					return FALSE;
			...
			{
				Trap_cddFPGAWorkState_T *pDynamicData = &(trap_data_p->u.cdd_FPGAState_trap );
				memcpy(enterprise, vt3888Notifications_oid, len_vt3888Notifications_oid * sizeof(oid));
				enterprise_length = len_vt3888Notifications_oid;
				trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
				specific = 1;
				...
				if(!snmp_varlist_add_variable( &vars, cdd_FPGA_Work_State_Trap_oid, len_cdd_FPGA_Work_State_Trap_oid, ASN_OCTET_STR, 
					(const void *) (pDynamicData->cddFPGAWorkState), strlen(pDynamicData->cddFPGAWorkState)))
				{
					snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
					snmp_free_varbind(vars);
					return FALSE;
			...
			{
				Trap_cddNetLinkState_T *pDynamicData = &(trap_data_p->u.cdd_NetLinkState_trap );
				memcpy(enterprise, vt3888Notifications_oid, len_vt3888Notifications_oid * sizeof(oid));
				enterprise_length = len_vt3888Notifications_oid;
				trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
				specific = 1;
				...
				if(!snmp_varlist_add_variable( &vars, cdd_Net_Link_Trap_oid, len_cdd_Net_Link_Trap_oid, ASN_OCTET_STR, 
					(const void *) (pDynamicData->cddNetLinkState), strlen(pDynamicData->cddNetLinkState)))
				{
					snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
					snmp_free_varbind(vars);
					return FALSE;
			...
			{
				Trap_cddOmapRunState_T *pDynamicData = &(trap_data_p->u.cdd_OmapRunState_trap );
				memcpy(enterprise, vt3888Notifications_oid, len_vt3888Notifications_oid * sizeof(oid));
				enterprise_length = len_vt3888Notifications_oid;
				trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
				specific = 1;
				...
				if(!snmp_varlist_add_variable( &vars, cdd_Omap_Run_Trap_oid, len_cdd_Omap_Run_Trap_oid, ASN_OCTET_STR, 
					(const void *) (pDynamicData->cddOmapRunState), strlen(pDynamicData->cddOmapRunState)))
				{
					snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
					snmp_free_varbind(vars);
					return FALSE;
...
#if VT_3830_MIB
        case SNMP_TRAP_QUEUE_BTSALARMSTATE:
        {
            Trap_btsEquipment_T *pDynamicData = &(trap_data_p->u.bts_equipment_trap);
            memcpy(enterprise, vt3830Notifications_oid, len_vt3830Notifications_oid * sizeof(oid));
            enterprise_length = len_vt3830Notifications_oid;
            //enterprise_length= 0;
            trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
            specific = 1;
            vars = NULL;
            /*if(!snmp_varlist_add_variable( &vars, ID_btsAlarmStatus, len_btsAlarmStatus, ASN_INTEGER, 
                (const void *) &(pDynamicData->btsAlarmStatus), sizeof(pDynamicData->btsAlarmStatus)))
            {
                printf("add var fail%s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            }*/

            if(!snmp_varlist_add_variable( &vars, ID_btsAlarmState, len_btsAlarmState, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsAlarmType), strlen(pDynamicData->btsAlarmType)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_vigorSource, len_vigorSource, ASN_OCTET_STR, 
                (const void *) (pDynamicData->vigorSource), strlen(pDynamicData->vigorSource)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
        ...
        case SNMP_TRAP_QUEUE_BTSLINKSTATE:
        {
            Trap_btsLinkState_T *pDynamicData = & trap_data_p->u.bts_linkstate_trap;
            memcpy(enterprise, vt3830Notifications_oid, len_vt3830Notifications_oid * sizeof(oid));
            enterprise_length = len_vt3830Notifications_oid;
            trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
            specific = 2;
            vars = NULL;
            if(!snmp_varlist_add_variable( &vars, ID_btsLinkState, len_btsLinkState, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsLinkState), strlen(pDynamicData->btsLinkState)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
        ...
        case SNMP_TRAP_QUEUE_BTSEXTCLOCK:
        {
            Trap_btsExtClock_T *pDynamicData = & trap_data_p->u.bts_extclock_trap;
            memcpy(enterprise, vt3830Notifications_oid, len_vt3830Notifications_oid * sizeof(oid));
            enterprise_length = len_vt3830Notifications_oid;
            trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
            specific = 3;
            vars = NULL;
            if(!snmp_varlist_add_variable( &vars, ID_btsExtClock, len_btsExtClock, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsExtClock), strlen(pDynamicData->btsExtClock)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
        ...
        case SNMP_TRAP_QUEUE_BTSTXSTATE:
        {
            Trap_btsState_T *pDynamicData = & trap_data_p->u.bts_txstate_trap;
            memcpy(enterprise, vt3830Notifications_oid, len_vt3830Notifications_oid * sizeof(oid));
            enterprise_length = len_vt3830Notifications_oid;
            trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
            specific = 4;
            vars = NULL;
            if(!snmp_varlist_add_variable( &vars, ID_btsTxState, len_btsTxState, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsTxState), strlen(pDynamicData->btsTxState)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_btsRealTxPower, len_btsRealTxPower, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsRealTxPower), strlen(pDynamicData->btsRealTxPower)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_btsVswr, len_btsVswr, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsVswr), strlen(pDynamicData->btsVswr)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_btsRssi, len_btsRssi, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsRssi), strlen(pDynamicData->btsRssi)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_btsEnvirTemperature, len_btsEnvirTemperature, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsEnvirTemperature), strlen(pDynamicData->btsEnvirTemperature)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_btsPaTemperature, len_btsPaTemperature, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsPaTemperature), strlen(pDynamicData->btsPaTemperature)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_btsDcVoltage, len_btsDcVoltage, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsDcVoltage), strlen(pDynamicData->btsDcVoltage)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_btsBatVoltage, len_btsBatVoltage, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsBatVoltage), strlen(pDynamicData->btsBatVoltage)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_btsFanSpeed, len_btsFanSpeed, ASN_OCTET_STR, 
                (const void *) (pDynamicData->btsFanSpeed), strlen(pDynamicData->btsFanSpeed)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
...
#if VT_3308_MIB
        case SNMP_TRAP_QUEUE_SCCSLOTSTATE:
        {
            Trap_mscSlotState_T *pDynamicData = & trap_data_p->u.msc_slotstate_trap;
            memcpy(enterprise, vt3308Notifications_oid, len_vt3308Notifications_oid * sizeof(oid));
            enterprise_length = len_vt3308Notifications_oid;
            trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
            specific = 1;
            vars = NULL;
            if(!snmp_varlist_add_variable( &vars, ID_sccSlotNum, len_sccSlotNum, ASN_OCTET_STR, 
                (const void *) (pDynamicData->sccSlotNum), strlen(pDynamicData->sccSlotNum)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_sccBusy, len_sccBusy, ASN_OCTET_STR, 
                (const void *) (pDynamicData->sccBusy), strlen(pDynamicData->sccBusy)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_sccCaller, len_sccCaller, ASN_OCTET_STR, 
                (const void *) (pDynamicData->sccCaller), strlen(pDynamicData->sccCaller)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_sccCalled, len_sccCalled, ASN_OCTET_STR, 
                (const void *) (pDynamicData->sccCalled), strlen(pDynamicData->sccCalled)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_sccRtpIpAddress, len_sccRtpIpAddress, ASN_OCTET_STR, 
                (const void *) pDynamicData->sccRtpIpAddress, strlen(pDynamicData->sccRtpIpAddress)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_sccCallInfo, len_sccCallInfo, ASN_OCTET_STR, 
                (const void *) pDynamicData->sccCallInfo, strlen(pDynamicData->sccCallInfo)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
	     ...
	     if(!snmp_varlist_add_variable( &vars, ID_sccBSID, len_sccBSID, ASN_OCTET_STR, 
                (const void *) pDynamicData->sccBSID, strlen(pDynamicData->sccBSID)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
...
#if 0
        case SNMP_TRAP_QUEUE_SCCBTSSTATE:
        {
            Trap_mscBtsState_T *pDynamicData = & trap_data_p->u.msc_btsstate_trap;
            memcpy(enterprise, vt3308Notifications_oid, len_vt3308Notifications_oid * sizeof(oid));
            enterprise_length = len_vt3308Notifications_oid;
            trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
            specific = 2;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_mscBtsIpAddress, len_mscBtsIpAddress, ASN_OCTET_STR, 
                (const void *) pDynamicData->sccBtsIpAddress, sizeof(pDynamicData->sccBtsIpAddress)))
            {
                printf("add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_mscBtsState, len_mscBtsState, ASN_OCTET_STR, 
                (const void *) (pDynamicData->sccBtsState), sizeof(pDynamicData->sccBtsState)))
            {
                printf("add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_vigorSource, len_vigorSource, ASN_OCTET_STR, 
                (const void *) (pDynamicData->vigorSource), sizeof(pDynamicData->vigorSource)))
            {
                printf("add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
        ...
        case SNMP_TRAP_QUEUE_SCCLINKSTATE:
        {
            Trap_mscLinkState_T *pDynamicData = & trap_data_p->u.msc_linkstate_trap;
            memcpy(enterprise, vt3308Notifications_oid, len_vt3308Notifications_oid * sizeof(oid));
            enterprise_length = len_vt3308Notifications_oid;
            trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
            specific = 3;
            vars = NULL;
            if(!snmp_varlist_add_variable( &vars, ID_mscLinkState, len_mscLinkState, ASN_OCTET_STR, 
                (const void *) (pDynamicData->sccLinkState), strlen(pDynamicData->sccLinkState)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
        ...
        case SNMP_TRAP_QUEUE_SCCPTTSTATETRAP:
        {
            Trap_sccPttState_T *pDynamicData = & trap_data_p->u.scc_PttState_Trap;
            memcpy(enterprise, vt3308Notifications_oid, len_vt3308Notifications_oid * sizeof(oid));
            enterprise_length = len_vt3308Notifications_oid;
            trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
            specific = 4;
            ...
            if(!snmp_varlist_add_variable( &vars, ID_sccTalker, len_sccTalker, ASN_OCTET_STR, 
                (const void *) (pDynamicData->sccTalker), strlen(pDynamicData->sccTalker)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
...
#endif
        case SNMP_TRAP_QUEUE_VIGORHEARTBEAT:
        {
            Trap_vigorHeartbeat_T *pDynamicData = & trap_data_p->u.vigor_heartbeat_trap;
            memcpy(enterprise, vigorPubNotifications_oid, len_vigorPubNotifications_oid * sizeof(oid));
            enterprise_length = len_vigorPubNotifications_oid;
            trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
            specific = 1;
...
#if (VT_3830_MIB == TRUE)
            if(!snmp_varlist_add_variable( &vars, ID_vigorEquipmentType, len_vigorEquipmentType, ASN_OCTET_STR, 
                (const void *) (pDynamicData->equipType), strlen(pDynamicData->equipType)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
            }
#elif(VT_3308_MIB == TRUE)
            if(!snmp_varlist_add_variable( &vars, ID_vigorEquipmentType, len_vigorEquipmentType, ASN_OCTET_STR, 
                (const void *) (pDynamicData->equipType), strlen(pDynamicData->equipType)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
...
#if (VT_3830_MIB == TRUE)
            if(!snmp_varlist_add_variable( &vars, ID_vigorDeviceState, len_vigorDeviceState, ASN_OCTET_STR, 
                (const void *) (pDynamicData->vigorDeviceState), strlen(pDynamicData->vigorDeviceState)))
            {
                snmp_log(LOG_DEBUG,"add var fail %s %d\r\n",__FUNCTION__,__LINE__);
                snmp_free_varbind(vars);
                return FALSE;
        ...
        case SNMP_TRAP_QUEUE_VIGORPROCABORT:
        {
            Trap_vigorProcAbort_T *pDynamicData = & trap_data_p->u.vigor_procabort_trap;
            memcpy(enterprise, vigorPubNotifications_oid, len_vigorPubNotifications_oid * sizeof(oid));
            enterprise_length = len_vigorPubNotifications_oid;
            trap_type = SNMP_TRAP_ENTERPRISESPECIFIC;
            specific = 2;
        ...
        default :
			snmp_log(LOG_DEBUG,"send trap error %s %d\r\n",__FUNCTION__,__LINE__);
            return FALSE;
    }
            ...
            enterprise_length, vars, NULL, 0))
    {
        snmp_log(LOG_DEBUG,"send trap fail %s %d\r\n",__FUNCTION__,__LINE__);
			...
			snmp_log(LOG_DEBUG,"send trap Sucess %s %d type[%d]\r\n",__FUNCTION__,__LINE__,trap_data_p->trap_type);
		}
	//pthread_mutex_unlock(&mut_trap);
    snmp_free_varbind(vars);
...
static uint32_t   snmp_trap_sem_id;
static SNMP_TRAP_Queue_T trap_queue;


uint32_t SNMP_TRAP_CreateSem(uint32_t sem_key, uint32_t sem_cnt, uint32_t *sem_id)
{
    uint32_t   ret;
	union semun arg;
	if(sem_id == NULL)
	...
	if(sem_key == 0)
		ret = semget(IPC_PRIVATE, 1, 0777 | IPC_CREAT | IPC_EXCL);
	else
		ret = semget((key_t)sem_key, 1, 0777 | IPC_CREAT | IPC_EXCL);

	if(ret < 0)
	{
	    snmp_log(LOG_DEBUG,"semget fail error:[]  %s %d \r\n",errno, __FUNCTION__,__LINE__);
		return -1;
	}

	*sem_id = ret;

	arg.val = (sem_cnt > 0)?1 : 0;
	ret = semctl((int)*sem_id, 0, SETVAL, arg);
	if(ret < 0)
		return -1;
...
uint32_t SNMP_TRAP_TakeSem(uint32_t sem_id)
{
    uint32_t  rc;
    struct sembuf sop= {0};
	...
	if(rc == -1)
	{
	    snmp_log(LOG_DEBUG,"sem errno : %d, %s %d\r\n",errno, __FUNCTION__,__LINE__);
		return -1;
	}
...
uint32_t SNMP_TRAP_GiveSem(uint32_t sem_id)
{
    union semun arg;
	uint32_t    rc;
...
{
    if(SNMP_TRAP_CreateSem(0, 1, &(trap_queue.sem)) != 0)
    {
        snmp_log(LOG_DEBUG,"CreateSem fail %s %d \r\n",__FUNCTION__,__LINE__);
        return;
    }
	trap_queue.buf = malloc(SNMP_TRAP_MAX_QUE_BUF_SIZE);
	if(trap_queue.buf == NULL)
	{
	    snmp_log(LOG_DEBUG,"malloc fail %s %d \r\n",__FUNCTION__,__LINE__);
	}
	memset(trap_queue.buf, 0, SNMP_TRAP_MAX_QUE_BUF_SIZE);
	trap_queue.buf->magic = ~0;
	trap_queue.buf->size = SNMP_TRAP_MAX_QUE_BUF_SIZE;
}

static TRAP_EVENT_TrapQueueData_T *SNMP_TRAP_BuildQueueData(TRAP_EVENT_TrapData_T *trapData)
{
    TRAP_EVENT_TrapQueueData_T         *new_blk;
	uint32_t  dynamicDatasize;

	dynamicDatasize = sizeof(trapData->u);

	new_blk = malloc(sizeof(TRAP_EVENT_TrapQueueData_T) + dynamicDatasize);
	if(NULL == new_blk)
	{
	    return NULL;
	}
	memcpy(new_blk, trapData, sizeof(TRAP_EVENT_TrapQueueData_T));
	memcpy(new_blk->data, &(trapData->u),  dynamicDatasize);
	return new_blk;
}

uint32_t SNMP_TRAP_GetTrapDataSize(TRAP_EVENT_TrapData_T *trap)
{
    return 0;
}

uint32_t SNMP_TRAP_GetTrapQueueBufDataSize(SNMP_TRAP_Queue_BUF_T *buf)
{
	...
	{
	    size = buf->size - rd_off + wr_off;
	}
	else if(buf->flags & SNMP_TRAP_QUEUE_FULL)
	{
	    size = buf->size;
	}
    return size;
...
void SNMP_TRAP_QueueBufReset(SNMP_TRAP_Queue_BUF_T *queue_buf)
{
    if(NULL == queue_buf)
		return;
	...
	queue_buf->wr_off = 0;
	queue_buf->rd_off = 0;
	queue_buf->mtime = time(NULL);
}

int SNMP_TRAP_QueueBufRead(SNMP_TRAP_Queue_BUF_T *queue_buf,  uint32_t offset, uint8_t *buf, int size, int flag)
{
    uint32_t datalen = 0;

	if(NULL == queue_buf || size < 0)
		return -1;

	if(!size || ((datalen = SNMP_TRAP_GetTrapQueueBufDataSize(queue_buf)) <= offset))
		return 0;
	...
	if(buf)
	{
	    uint8_t * base = (uint8_t *)(queue_buf + 1);
		uint32_t  rd_off = (queue_buf->rd_off + offset) % queue_buf->size;

		if((rd_off + datalen) <= queue_buf->size)
		{
		    memmove((void *)buf, (void *)(base + rd_off), (int)datalen);
		}
		else
		{
		    int len1= (int)(queue_buf->size - rd_off);
			int len2 = (int)(datalen - len1);
	...
	if(!(flag & SNMP_TRAP_QUEUE_PEEK))
	{
	    queue_buf->rd_off = (queue_buf->rd_off + offset + datalen) % queue_buf->size;
		if(datalen && (queue_buf->flags & (SNMP_TRAP_QUEUE_FULL | SNMP_TRAP_QUEUE_OVERLAP)))
			queue_buf->flags &= ~(SNMP_TRAP_QUEUE_FULL | SNMP_TRAP_QUEUE_OVERLAP);
		queue_buf->mtime =  time(NULL);
	}
...
int SNMP_TRAP_QueueBufWrite(SNMP_TRAP_Queue_BUF_T *queue_buf,  uint8_t *buf, int size, int flag)
{
    uint32_t  space;
	if(NULL == queue_buf || (size < 0))
		return -1;
	...
	space = queue_buf->size - SNMP_TRAP_GetTrapQueueBufDataSize(queue_buf);
	if(space < (uint32_t)size)
	{
	    snmp_log(LOG_DEBUG,"no space to write trap %s %d\r\n",__FUNCTION__,__LINE__);
	    if(!(flag & SNMP_TRAP_QUEUE_OVER_WRITE))
	    {
	        if(!space)
	        {
	            ++queue_buf->errcnt;
		...
		if(queue_buf->size == size)
		{
		    SNMP_TRAP_QueueBufReset(queue_buf);
			space = queue_buf->size;
			goto write_data;
		}

		while((space < size) && queue_buf->entries)
		{
		    uint16_t nbytes;
			SNMP_TRAP_QueueBufRead(queue_buf, 0, (uint8_t *)&nbytes, (int)(sizeof(nbytes)), SNMP_TRAP_QUEUE_PEEK);
            SNMP_TRAP_QueueBufRead(queue_buf, 0, NULL, (int)nbytes, 0);
			--queue_buf->entries;
			space += nbytes;
			
		}
		queue_buf->flags |= SNMP_TRAP_QUEUE_OVERLAP;
	}
...
write_data:
	...
	{
	    uint8_t *base = (uint8_t *)(queue_buf + 1);
		uint32_t wr_off = queue_buf->wr_off;

		if((wr_off + size) <= queue_buf->size)
		{
		    memmove((void *)(base + wr_off), (void *)buf, size);
		}
		else
		{
		    int len1 = (int)(queue_buf->size - wr_off);
			int len2 = (int)(size - len1);
	...
	queue_buf->wr_off = (queue_buf->wr_off + size) % queue_buf->size;
	if(queue_buf->wr_off == queue_buf->rd_off)
		queue_buf->flags |= SNMP_TRAP_QUEUE_FULL;
	queue_buf->mtime = time(NULL);
}

int SNMP_TRAP_QueueEnqueue(TRAP_EVENT_TrapData_T *trap, SNMP_TRAP_Queue_T *q, int flag)
{
    uint32_t    total_size, comm_size, data_size;
	uint32_t    buf_datasize, space;
	int         ret;
	SNMP_TRAP_QUEUE_ENTRY_T entry;
	
    if(trap == NULL)
    {
        snmp_log(LOG_DEBUG,"qData = NULL  %s %d \r\n",__FUNCTION__,__LINE__);
        return FALSE;
    }

	if(q == NULL || q->buf == NULL)
	{
	    snmp_log(LOG_DEBUG,"q = NULL  %s %d \r\n",__FUNCTION__,__LINE__);
        return FALSE;
	}
	...
	data_size = SNMP_TRAP_GetTrapDataSize(trap);

	comm_size = ((trap->community_specified) ? (strlen((char *)(trap->community)) + 2) : 0);
	total_size = (int)(SNMP_TRAP_QUEUE_ENTRY_HDR_SIZE + comm_size + data_size);

	if(comm_size && ((comm_size - 2) > SNMP_TRAP_MAX_COMM_STR_NAME_LEN))
	{
	    snmp_log(LOG_DEBUG,"community string size error %s %d\r\n",__FUNCTION__,__LINE__);
		return -1;
	}

    SNMP_TRAP_TakeSem(q->sem);
	buf_datasize = SNMP_TRAP_GetTrapQueueBufDataSize(q->buf);
	space = q->buf->size - buf_datasize;
	if(space < total_size)
	{
	   snmp_log(LOG_DEBUG,"no enough free space for trap %lu size %d\r\n",trap->trap_type, data_size);
	   if(!(flag & SNMP_TRAP_QUEUE_OVER_WRITE) ||
	   	(q->buf->size < total_size))
	   	{
	   	    ret = -1;
			goto write_exit;
	   	}
	...
	entry.size = (uint16_t)total_size;
	entry.type = (uint16_t)trap->trap_type;
	entry.time = (uint32_t)trap->trap_time;
	entry.retries = (uint8_t)trap->remainRetryTimes;
	if(trap->community_specified)
	{
	   entry.flags |= SNMP_TRAP_QUEUE_ENTRY_COMMUNITY;
	   entry.content.comm.len = (uint8_t)(comm_size - 1);
	   ret += SNMP_TRAP_QueueBufWrite(q->buf, (uint8_t *)&entry, (SNMP_TRAP_QUEUE_ENTRY_HDR_SIZE + 1), flag);
	   ret += SNMP_TRAP_QueueBufWrite(q->buf, (uint8_t *)trap->community, entry.content.comm.len, flag);
	}
	else
	{
	    ret += SNMP_TRAP_QueueBufWrite(q->buf, (uint8_t *)&entry, (SNMP_TRAP_QUEUE_ENTRY_HDR_SIZE ), flag);
	}
	ret += SNMP_TRAP_QueueBufWrite(q->buf, (uint8_t *)&(trap->u), data_size, flag);

	++q->buf->entries;
	if(q->buf->entries > q->buf->max_entries)
		q->buf->max_entries = q->buf->entries;

	buf_datasize += total_size;
	if(buf_datasize > q->buf->max_size)
		q->buf->max_size = buf_datasize;

write_exit:
	SNMP_TRAP_GiveSem(q->sem);
	return ret;
}
...
int SNMP_TRAP_QueueDequeue(TRAP_EVENT_TrapData_T *trap, SNMP_TRAP_Queue_T *q, int n, int flag)
{
    uint32_t    offset, nx, ret;
	SNMP_TRAP_QUEUE_ENTRY_T entry;
	...
	{
	    memset(& entry, 0, sizeof(entry));
		if(ret = SNMP_TRAP_QueueBufRead( q->buf, offset, (uint8_t *)&entry, 
			      (SNMP_TRAP_QUEUE_ENTRY_HDR_SIZE + 1), SNMP_TRAP_QUEUE_PEEK) <= 0)
		{
		    break;
		}
		if(trap && (ret > 0))
		{
			...
			trap->trap_time  = entry.time;
			trap->remainRetryTimes = entry.retries;
			trap->trap_type = entry.type;
			trap->community_specified = (BOOL)((entry.flags & SNMP_TRAP_QUEUE_ENTRY_COMMUNITY) != 0);
			trap->flag  = entry.flags;
			trap->community[0] = 0;

			if(trap->community_specified)
			{
			    int comm_len = entry.content.comm.len;
				int comm_off = offset +SNMP_TRAP_QUEUE_ENTRY_HDR_SIZE + 1;
				if((ret = SNMP_TRAP_QueueBufRead(q->buf, comm_off, (uint8_t *)trap->community, comm_len, SNMP_TRAP_QUEUE_PEEK)) < 0)
				{
				    break;
				}
				data_len = entry.size - SNMP_TRAP_QUEUE_ENTRY_HDR_SIZE - 1 - comm_len;
				data_off = offset + SNMP_TRAP_QUEUE_ENTRY_HDR_SIZE + 1 + comm_len;
			}
			else
			{
			    data_len = entry.size - SNMP_TRAP_QUEUE_ENTRY_HDR_SIZE;
				data_off = offset + SNMP_TRAP_QUEUE_ENTRY_HDR_SIZE;
			}

			if((ret = SNMP_TRAP_QueueBufRead(q->buf, data_off, (uint8_t *)&trap->u, data_len, SNMP_TRAP_QUEUE_PEEK)) < 0)
			{
			    break;
			}
			++trap;
	...
	if(nx)
	{
	    if((flag & SNMP_TRAP_QUEUE_PEEK))
	    {
	        SNMP_TRAP_QueueBufRead(q->buf, 0, NULL, offset, 0);
			if(q->buf->entries > nx)
			{
			    q->buf->entries -= nx;
			}
                 ...
                 netsnmp_pdu *pdu, snmp_callback callback, void *cb_data)
{
	...
	snmp_log(LOG_DEBUG, "sending message id#%ld reqid#%ld len %ld\r\n", pdu->msgid, pdu->reqid, length);
    ...
    if (result < 0) {
		...
		snmp_log(LOG_DEBUG, "sending message fail \r\n");
        return 0;
    }

	snmp_log(LOG_DEBUG, "sending message success id#%ld reqid#%ld len  %ld\r\n", pdu->msgid, pdu->reqid, length);
...
//pthread_mutex_lock(&mut_trap);
  if (netsnmp_ds_get_boolean(NETSNMP_DS_LIBRARY_ID,NETSNMP_DS_LIB_DUMP_PACKET)) {
	...
	char traptype[10]= {0};
	char trapreceiver[30]= {0};
	char *addr_p = NULL;
	memset(traptype, 0,sizeof(traptype));
	memset(trapreceiver, 0,sizeof(trapreceiver));
	netsnmp_session * ss;
	if (opaque != NULL && olength == sizeof(netsnmp_indexed_addr_pair))
	{
		addr_pair = (netsnmp_indexed_addr_pair *) opaque;
    } else if (transport != NULL && transport->data != NULL)
    {
		addr_pair = (netsnmp_indexed_addr_pair *) transport->data;
    }
	if(addr_pair != NULL)
	{
		memcpy(&addr, &(addr_pair->remote_addr), sizeof(addr));
		addr.sin_port  = htons((u_short)SNMP_TRAP_PORT);
	    ss = snmp_sess_find_by_addr(&addr);
		if(NULL == ss)
		{
		    addr_p = inet_ntoa(addr.sin_addr);
			if(addr_p != NULL)
			{
			    memcpy(traptype, "trap2sink", sizeof("trap2sink"));
				snprintf(trapreceiver, sizeof(trapreceiver), "%s:%d public", addr_p,SNMP_TRAP_PORT);
				snmp_log(LOG_DEBUG,"a %s  b %s\r\n",traptype,trapreceiver);
				is_NULL = FALSE;
				snmpd_parse_config_trap2sink(traptype, trapreceiver);
			}
                    ...
                    int incr_retries)
{
	...
	snmp_log(LOG_DEBUG, "resending message id#%ld reqid#%ld "
                "rp_reqid#%ld rp_msgid#%ld len %ld \r\n",
                rp->pdu->msgid, rp->pdu->reqid, rp->request_id, rp->message_id, length);
    result = netsnmp_transport_send(transport, packet, length,
    ...
    if (result < 0) {
        ...
        sp->s_errno = errno;
		snmp_log(LOG_DEBUG, "resending message fail id#%ld reqid#%ld "
                "rp_reqid#%ld rp_msgid#%ld len %ld\r\n" ,
                rp->pdu->msgid, rp->pdu->reqid, rp->request_id, rp->message_id, length);
        snmp_set_detail(strerror(errno));
        return -1;
        ...
        tv.tv_usec %= 1000000L;
        rp->expireM = tv;
		snmp_log(LOG_DEBUG, "resending message success id#%ld reqid#%ld "
                "rp_reqid#%ld rp_msgid#%ld len %ld\r\n" ,
                rp->pdu->msgid, rp->pdu->reqid, rp->request_id, rp->message_id, length);
    }
    return 0;
